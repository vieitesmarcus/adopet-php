...
class CadastrarUserController implements RequestHandlerInterface
{
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if ($request->getQueryParams() && key_exists('confirm', $request->getQueryParams())) {
            ...
            $getRepositoryUser = $entityManager->getRepository(User::class);
            $obUser = $getRepositoryUser->findOneBy(['email' => $emailDescriptografado]);
            /** @var User $obUser */
            $obUser->setMailValidation(1);
//            $entityManager->persist($obUser);
        ...
        if ($request->getParsedBody()) {
            ...
            $user['user']['confirmPassword'] = filter_var($user['user']['confirmPassword'],
            ...
            $regexName = '/^[a-zA-Z ]*$/';
            if (!isset($user['user']['name']) || strlen($user['user']['name']) < 3 || preg_match($regexName,
                    $user['user']['name']) === 0) {
                $obErro->addMessage($obErro::NAME,
                    "não use caracteres especiais no nome e/ou nome com menos de 3 carateres.");
            }
            ...
            $obUser->setMailValidation(0);
            $obUser->setCreated_at(new \DateTime('now'));
            $obUser->setUpdated_at(new \DateTime('now'));
...
//            var_dump($user, $emailCriptografado, $obEmail);exit();
